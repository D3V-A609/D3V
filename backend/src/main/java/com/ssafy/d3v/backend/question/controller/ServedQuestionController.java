...
@Tag(name = "제공된 질문", description = "제공된 질문 관련 API")
public class ServedQuestionController {

    private final ServedQuestionService servedQuestionService;

    @GetMapping
    @Operation(summary = "제공된 질문 전체 조회", description = "모든 제공된 질문을 조회합니다.")
    ...
    @GetMapping("/{id}")
    @Operation(summary = "제공된 질문 하나 조회", description = "주어진 제공된 질문 ID에 해당하는 제공된 질문을 조회합니다.")
    ...
    @PostMapping
    @Operation(summary = "제공된 질문 생성", description = "제공된 질문을 생성합니다.")
    public ResponseEntity<ServedQuestionDto> createServedQuestion(
            @RequestBody ServedQuestionCreateRequest servedQuestionCreateRequest) {
    ...
    @Operation(summary = "특정 멤버의 제공된 질문 전체 조회", description = "주어진 멤버 ID에 해당하는 제공된 질문 전체를 조회합니다. 내 질문을 조회할 때도 사용")
